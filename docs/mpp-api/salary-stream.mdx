...
The stream emits a single employee's accrual — caller must be the employee (Privy) or the employer / authorized agent that owns them. See [Authentication](/docs/mpp-api/authentication#two-factor-payment--principal).
